...
export class IncidentCollaborationRecordRoom extends RecordRoom<Env> {
  ...
  private async tool(userId: string, tool: string, params: Record<string, unknown>): Promise<ActionResult<Record<string, unknown>>> {
  ...
  override async fetch(request: Request): Promise<Response> {
    ...
    } catch { return fail('Could not confirm the change. Reopen the incident to check its saved state before retrying.', 503) }
  }
