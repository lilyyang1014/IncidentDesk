...
import { handleInvitation, hasPendingInvitation, revokeAcceptedInvitation } from './incident-invitations'
...
export class IncidentCollaborationRecordRoom extends RecordRoom<Env> {
  ...
  override async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (invitation?.success && (invitation.data.intent === 'inspect' || invitation.data.intent === 'accept')) {
        // Auth service reads happen outside the shared room gate. No client
        // supplied profile/header can replace this independent verification.
        try { recipientEmail = await verifiedInvitationEmail(this.env, auth, invitationSessionCookie(request.headers.get('Cookie'))) }
        catch (error) { return fail(error instanceof Error ? error.message : 'Could not verify your sign-in.', 403) }
      }
      ...
      return await this.state.blockConcurrencyWhile(async () => {
        ...
        if (input.intent === 'add' || input.intent === 'remove') {
          ...
          if (input.intent === 'add') {
            ...
            if (hasPendingInvitation(this.sql, source)) return fail('Cancel the pending invitation before adding a registered collaborator.')
            ...
            revokeAcceptedInvitation(this.sql, source, input.userId)
            if (!existing) { this.refreshAccess(); return Response.json({ success: true }) }
