...
async function write(input: IncidentWriteRequest): Promise<string> {
  ...
  if (!result.success) throw new Error(result.error ?? 'Could not save the incident.')
  if (!result.data?.recordId) throw new Error('Mutation confirmation timed out')
  return result.data.recordId
}
const responseSchema = z.object({ success: z.boolean(), uncertain: z.boolean().optional(), error: z.string().optional(), data: z.object({ recordId: z.string() }).optional() })
