...
const issuer = 'https://incident-ai.test'
const tokens = {}
...
const persistence = await mkdtemp(join(tmpdir(), 'incident-ai-runtime-'))
let providerCalls = 0
const options = {
  ...
  serviceBindings: {
    API_WORKER: async (request) => {
      ...
      return Response.json({ success: true, data: { model: 'test-provider', choices: [{ finish_reason: 'stop', message: { content: JSON.stringify({
        summary: 'The log contains a timeout.', evidence: [{ line: 3, quote: '  request timed out  ' }],
        hypotheses: [{ explanation: 'A dependency may be slow.', evidenceLines: [3] }], suggestedChecks: ['Check dependency latency.'],
      }) } }] } })
    },
  },
}
...
async function action(user, intent) {
  const response = await runtime.dispatchFetch('http://local.test/api/actions/analyzeIncident', {
    method: 'POST', headers: { Authorization: `Bearer ${tokens[user]}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ incidentId: 'test-incident', intent }),
  })
...
try {
  ...
  assert.equal((await action('member-b', 'generate')).success, false)
  assert.equal(providerCalls, 0)
  assert.equal((await action('member-a', 'status')).data.phase, 'idle')
  const replies = await Promise.all([action('member-a', 'generate'), action('member-a', 'generate')])
  ...
  assert.equal(restored.data.result.evidence[0].line, 3)
  assert.equal((await action('member-b', 'status')).success, false)
  assert.equal((await action('owner', 'status')).data.phase, 'complete')
  assert.equal((await action('member-a', 'generate')).data.phase, 'complete')
  assert.equal(providerCalls, 1)
