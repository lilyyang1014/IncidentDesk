...
Select **Preview handoff report** in an incident detail to open a read-only draft containing the incident title, ID, creation time, local analysis status, saved AI summary/evidence/hypotheses/checks, the latest submitted reference query's saved links/excerpts and original logs.

The browser reads the existing authenticated AI and reference status actions concurrently. Neither request generates analysis, searches Exa, changes incident records or sends email. Both actions retain their existing server-side membership and creator/admin checks. A failed read blocks the draft with an error; a successful read with no completed result displays the actual phase instead of inventing content. The latest query can be incomplete even if an older query has saved results; the preview does not browse history.
